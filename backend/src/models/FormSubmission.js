...
const debtFormSubmissionSchema = new Schema({
  ...
  struggles: {
    type: [{
      type: String,
      enum: ['high-interest', 'min-payments', 'multiple-cards', 'medical-debt', 'all']
    }],
    validate: [
      (val) => val.length > 0,
  ...
  debtTypes: {
    type: [{
      type: String,
      enum: ['credit-cards', 'personal-loans', 'student-loans', 'medical-bills', 'auto-loans', 'taxes', 'other']
    }],
    validate: [
        (val) => val.length > 0,
  ...
  email: {
    ...
    trim: true,
    lowercase: true,
    match: [/^[^\s@]+@[^\s@]+\.[A-Za-z]{2,24}$/, 'Please fill a valid email address.']
  },
  option: {
    type: Boolean,
    required: true,
...
export const FormSubmission = mongoose.model('formSubmit', debtFormSubmissionSchema);